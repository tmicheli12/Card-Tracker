  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="page-title">Snipe Tool</h1>
        <button
          onClick={refresh}
        ...
        {showSearches && (
          <div className="mt-3 space-y-1.5">
            {searches.map((s) => (
              <div key={s.id} className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={s.enabled}
                  onChange={() => toggleSearch(s.id, s.enabled)}
                  className="w-4 h-4 accent-blue-500"
                />
                <span className={`flex-1 text-sm ${s.enabled ? 'text-zinc-200' : 'text-zinc-500 line-through'}`}>
                  {s.search_term}
                </span>
                <button
            ...
            <div className="flex gap-2 pt-3 border-t border-zinc-800 mt-3">
              ...
              />
              <button onClick={addSearch} className="btn-secondary text-xs px-3" disabled={!newTerm.trim()}>
                + Add
              </button>
            </div>
          </div>
      ...
      {errors.length > 0 && (
        <div className="card mb-3 border border-amber-900/40 bg-amber-900/20 text-xs">
          ...
          <ul className="text-amber-400/80 space-y-0.5">
            {errors.map((e) => (
              <li key={e.term}>
                <strong>{e.term}:</strong> {e.error.substring(0, 120)}
              </li>
            ))}
          </ul>
        </div>
      ...
      {!lastRefresh ? (
        <div className="card text-center py-10 text-zinc-500">
          {enabledCount === 0
            ? 'Add at least one search term above, then click Refresh.'
            : 'Click 🔄 Refresh to load tonight\'s auctions.'}
        </div>
      ) : items.length === 0 ? (
        <div className="card text-center py-10 text-zinc-500">
          No auctions match the criteria right now. Try refreshing later — eBay listings update constantly.
        </div>
      ) : (
        <div>
          ...
          <div className="space-y-2">
            {items.map((item) => (
              <SnipeRow key={item.itemId} item={item} />
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <a href={item.url} target="_blank" rel="noopener noreferrer" className="card-hover block">
      <div className="flex items-start gap-3">
        {item.imageUrl && (
          <img src={item.imageUrl} alt="" className="w-16 h-16 object-cover rounded-lg shrink-0 bg-zinc-800" />
        )}
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap text-sm">
            ...
            <span className={`${urgency} text-xs font-semibold`}>
